...
const useInfos = [
  {
    ...
    title: '说明',
    msgs: [
      '脚本自动更新, 只需下载一次, 支持断线重连, 支持顶号半小时后重连, 支持多开, 支持游戏闪退重启, 支持隐藏模拟器',
      '脚本目前支持商店购物, 竞技场, 冒险, 元素禁地, 秘境支配者, 活动图',
      '脚本可以同时设置多个任务, 例如冒险神话8-5刷30次, 水装备本12层刷50次, 活动图V-3刷100次等等, 脚本会依次执行任务, 中间会自动穿插进行竞技场及商店购物, 脚本目前支持设置20个任务, 元素本及装备本可设置采用单次战斗, 减少失败时损失的体力'
    ]
  },
  {
    id: 1,
    title: '下载',
    msgs: [
      '首先请加QQ群249946487(暗号: ace), 群文件下载脚本, 直接解压即可',
      '免费向群主获取注册码(请保存, 以免丢失)',
      '打开http://47.96.69.41, 注册用户, 注册成功自动跳转脚本页面',
      '在脚本页面右上角用户名处点击绑定, 输入注册码进行绑定',
  ...
  },
  {
    ...
    msgs: [
      ...
      '分辨率: 请确认为平板版-960*540, 蓝叠还需设置dpi为160, 不要自己拉伸窗口说一百遍'
    ]
  },
  {
    id: 4,
...
const attentions = [
  {
    ...
    msgs: [
      ...
      '脚本暂停: 在命令行窗口(窗口键+r输入cmd打开命令行)上方菜单栏右键->默认值->选项->取消勾选快速编辑模式',
      '无法绑定窗口: 如果是蓝叠模拟器务必启动游戏后再启动脚本, 其余可尝试将模拟器窗口置于最上方再启动脚本, 脚本启动后再用其他窗口遮盖模拟器'
    ]
  },
  {
    id: 2,
